...
class Auth
{
    ...
    public function check($name, $uid, $type = 1, $mode = 'url', $relation = 'or')
    {
        ...
        if ($this->_config['AUTH_NOT_ADD']) {
            $rule = strtolower(MODULE_NAME . '/' . CONTROLLER_NAME . '/' . ACTION_NAME);
            $res = M('AuthRule')->where("name = '$rule'")->select();
            if (empty($res)) {
                return true;
            }
        }
     ...
     * @param integer $type
     */
    public static function getAuthListS($uid, $type)
    {
        static $_authList = array(); //保存用户验证通过的权限列表
        $t = implode(',', (array)$type);
        if (isset($_authList[$uid . $t])) {
        ...
        //读取用户组所有权限规则
        $rules = M()->table('link_auth_rule')->where($map)->field('name')->select();
