...
class Oss {
    ...
    const OSS_HOST = 'oss-cn-qingdao.aliyuncs.com'; // oss-cn-hangzhou.aliyuncs.com
    ...
    private $config = array(
		...
		'endpoint'   => 'oss-cn-qingdao.aliyuncs.com', //阿里云Access Key Secret
        'bucket'     => 'buzhigk', //空间名称
        'timeout'    => 90, //超时时间
    );
    ...
    public function mkdir($savepath) {
        return true;
		if ($litpic!=""){
  			$litpic = "http://www.buzhi.com".str_replace("http://www.buzhi.com/","/",$litpic);
  		}
    }
    ...
    public function save($file, $replace = true) 
	{
		...
		try { 
			...
			$accessKeySecret = $this->config['access_key'];
			$endpoint 		 = $this->config['endpoint'];
			
			$ossClient = new OssClient($accessKeyId, $accessKeySecret, $endpoint); 
			
			$object = $this->rootPath . $file['savepath'] . $file['savename'];
			$fileBody = $file['tmp_name']; //file_get_contents($file['tmp_name']);
			
			//$response = $ossClient->putObject($bucket, $object, $content); 
			$response = $ossClient->uploadFile($bucket, $object, $fileBody, $options = NULL);
			
			return 200 === $response['status'] ? true : false;
