...
class Upload {
     ...
     * @return array|bool
     */
    public function uploadStream ($files, $name = '')
    {
        if(empty($files) || empty($files['file']) || empty($files['ext'])){
            $this->error = '没有上传的文件！';
            return false;
        }
        ...
        //路径
        $path = $this->rootPath . $this->savePath;

        //图片名称
        $name = empty($name) ? md5(time().mt_rand(10, 99)).".".$files['ext'] : $name;
        ...
        //通过二进制得到哦图片
        $newfile = fopen($dir, "w");
        fwrite($newfile, $files['file']);
        fclose($newfile);

        $imageinfo = getimagesize($dir);
        $data = [
