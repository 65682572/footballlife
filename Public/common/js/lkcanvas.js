...
//	showMes("鼠标X:"+event.x+"鼠标Y:"+event.y);
	...
	var canvaY=canva.offsetParent.offsetTop+canva.offsetTop+21;
							...
							if(oilist!=null&&inputs!=null){
								...
								for(var j=0;j<oilist.length;j++){
									if(oilist[j].oi==id){
										...
										theDevice = oilist[j];//设置当前操作输入源对象
									}
								}
							}
							ct_list[i].strokeColor = "red";
...
function getCanXY(id){
	try {
		...
		if(paper_middle!=null){
			...
			// console.log("x2:"+paper_middle.offsetLeft+"  y2:"+paper_middle.offsetTop);
		}
