...
var MY_CANVAS="deCan";
theDevice = new Object();// 当前操作输入源对象
...
var BC_IMG="/Public/common/api/img/bcg.png";//画布前景
var MBC_IMG="/Public/common/api/img/bc.jpg";//自定义画布背景
var CL_IMG="/Public/common/api/img/an1.gif";//关闭按钮
var LB_IMG="/Public/common/api/img/menu_100.png";//选项卡
var ADD_IMG="/Public/common/api/img/jr_5.png";//通道管理-添加对应关系
var DEL_IMG="/Public/common/api/img/cs_5.png";//通道管理-删除对应关系
var TRUE_IMG="/Public/common/api/img/d.png";//对
var FALSE_IMG="/Public/common/api/img/x.png";//错
var CL2_IMG="/Public/common/api/img/wgb.png";//关闭按错
var CL2D_IMG="/Public/common/api/img/wgb-1.png";//关闭按下状态图片
var MIN_IMG="/Public/common/api/img/wsx.png";//缩小图片
var MIND_IMG="/Public/common/api/img/wsx-1.png";//缩小按下图片
var MAX_IMG="/Public/common/api/img/wfd.png";//放大图片
var MAXD_IMG="/Public/common/api/img/wfd-1.png";//放大按下图片
		...
		this.init=function(){
			try {
				...
				$("body").on("mousedown","#"+LK_CANVAS, (function(e) {
					...
					// console.log("e.which:"+e.which+" id:"+this.id);
					...
					if(e.which==1){
						...
						isTouch = true; //1.0.11新版本
						dragImage(this);//1.0.11新版本
						LONG_DOWN = true;//1.0.11新版本
						LONG_EVENT = e;//1.0.11新版本
						...
						try {
							...
							if (e.which == 3&&bcType==1) {// 放大状态不能右键点击
								...
								try {//1.0.11新版本
									...
									document.createEvent("TouchEvent");//1.0.11新版本
									return;//判断如果是移动端不支持右键//1.0.11新版本
								} catch (e) {
									console.log("不支持滑动功能！");//1.0.11新版本
								}
							...
							var imageMenuData=new Array();
							imageMenuData = returnMune();//1.0.11新版本
	...
	try {
		var imageMenuData=new Array();
		 imageMenuData = [ [ {
				...
				text : showJSTxt(indexJSList,110),// 云台控制
				func : function() {
					var data = Object();
                    data.gn = 0; //窗口id
                    data.inputId = 0; //输入源id
                    var fileSeven = lc.onGetFIle(7).split('/');
                    fileSeven = JSON.parse(fileSeven[1]);
                    $.each(fileSeven, function (k, v) {
                    	if (v.outputId == rightId) {
                            data.gn = v.gn;
                            data.inputId = v.inputId;
                        }
					});
                    document.location = 'linkyview://data=' + JSON.stringify(data);
					// 菜单功能...
				}
			},{
				text :(addType == 0)? $st(js_2,103) : $st(js_2,103.5),// 开启添加窗口\关闭添加窗口
				func : function() {
					var wins=returnList(local_win);
					if(wins==null){
						upAlert($st(indexJSList,106));
...
//showMes("444");
			...
			, {
				...
				func : function() {
					try{
						...
						// alert(new
						// Date('2015/12/21').getDay());//根据时间判断星期
					}catch(e){
						// showMes("错误:"+e);
					}
		 ...
		 try {
				...
				document.createEvent("TouchEvent");
				imageMenuData[0].splice(2,1);
			} catch (e) {
				console.log("不支持滑动功能！");
			}
		 return imageMenuData;
	} catch (e) {
		showMes("returnMune:"+e);
	}
}
...
//}
function bindEvent() {//1.0.11新版本
	document.getElementById(LK_CANVAS).addEventListener('touchstart', touchSatrtFunc, false);
	document.getElementById(LK_CANVAS).addEventListener('touchmove', touchMoveFunc, false);
	document.getElementById(LK_CANVAS).addEventListener('touchend', touchEndFunc, false);
}
function removeEvent() {
	document.getElementById(LK_CANVAS).removeEventListener('touchstart', touchSatrtFunc, false);
	document.getElementById(LK_CANVAS).removeEventListener('touchmove', touchMoveFunc, false);
	document.getElementById(LK_CANVAS).removeEventListener('touchend', touchEndFunc, false);
...
function getDeventXY(eventId,pMouseX,pMouseY){//1.0.11新版本
	try {
		var canva=document.getElementById(eventId);
		var index=new Object();
		var hideX=document.body.scrollLeft;//获取隐藏的宽度
		var hideY=document.body.scrollTop;//获取隐藏的高度
		var mouseX=pMouseX;//鼠标相对页面开始的X坐标
		var mouseY=pMouseY;//鼠标相对页面开始的Y坐标
		...
		console.log("getDeventXY:"+e);
		return null;
	}
}
var tsX = 0;//开始的X坐标//1.0.11新版本
var tsY = 0;//开始的Y坐标//1.0.11新版本
var moX = 0;//移动中的X坐标//1.0.11新版本
var moY = 0;//移动中的Y坐标//1.0.11新版本
var isTouch=false;//1.0.11新版本
var touchParam=null;//触摸框选的窗口//1.0.11新版本
var touchTime = 0;//开始触摸时间//1.0.11新版本
var touchTimeOut ;//1.0.11新版本
var tEvent = null; //当前触摸鼠标//1.0.11新版本
/*******************************************************************************
 * 滑动按下事件
 */
function touchSatrtFunc(evt) {//1.0.11新版本
	
	var touch = evt.touches[(evt.touches.length - 1)]; // 获取第一个触点
		var x = Number(touch.pageX); // 页面触点Y坐标
		var y = Number(touch.pageY); // 页面触点Y坐标
		tsX=x;
		tsy=y;
		var obj= getDeventXY(LK_CANVAS,x,y);
		tEvent = evt;
		evt.preventDefault(); 	
		LONG_EVENT = new Object();
		LONG_EVENT.clientX = x;
		LONG_EVENT.clientY = y;
//		isLongTouch();
		
		//tsX=tsX-输入源div
		var id=getClickObjId(obj.x,obj.y);
		if(id!=null){
...
//				clickObj(evt);
				...
				touchTimeOut = setTimeout(function(){isLongTouch();}  , 1000);
			}
			
		} catch (e) {
			console.log("不支持滑动功能！");
		}
	var myDate = new Date();//时间函数
	touchTime = myDate.getMilliseconds()+(myDate.getSeconds()*1000)+(myDate.getMinutes()*1000*60)+(myDate.getHours()*1000*60*60);
	
	//	console.log("触摸按下:(x:"+x+",y:"+y+") obj:"+JSON.stringify(obj)+" id:"+JSON.stringify(id));
}

/*******************************************************************************
 * 滑动移动事件
 */
function touchMoveFunc(evt) {//1.0.11新版本
	isTouch = sessionStorage.getItem("isTouch");
	if(isTouch){
		tEvent = evt;
		...
		moX = x;
		moY = y;
		var cElement = document.getElementById(LK_CANVAS);
		var offLeft = cElement.offsetLeft;
		var offTop = cElement.offsetTop;
		var param=new Object();
		param.x1=tsX-offLeft;//获取最小X坐标
		param.y1=tsy-offTop;//获取最小Y坐标
		param.x2=x-offLeft;//获取最大X坐标
		param.y2=y-offTop;//获取最大Y坐标
		param.lineWidth=2;
 ...
 * 滑动结束事件
 */
function touchEndFunc(evt) {//1.0.11新版本
		
//	evt.preventDefault(); // 阻止触摸时浏览器的缩放、滚动条滚动等
		var param=touchParam;
		...
		if(LONG_DOWN){
			...
			var touch = tEvent.touches[(tEvent.touches.length - 1)]; // 获取第一个触点
			var x = Number(touch.pageX); // 页面触点Y坐标
			var y = Number(touch.pageY); // 页面触点Y坐标
			var obj= getDeventXY(LK_CANVAS,x,y);
			var id=getClickObjId(obj.x,obj.y);
		...
		touchParam=null;//清空选择窗口
		if(param != null){ //拖动选择多窗口
 ...
 * 判断是否是长按事件
 */
function isLongTouch(){//1.0.11新版本
	try {
//		console.log("进行判断:"+ isTouch+  " moX:" +moX +" tsX:"+ tsX+"  moY:"  +moY  + "  tsY:" + tsY);
//		if(isTouch && (moX < 10 && moY  < 10)){
		...
		if(xmlEntities != null){
			...
			if(redNum < 2){//已选中的窗口大于一个表示合并状态，不提供点击选择窗口
				clickObj(tEvent);
			}
		}
 ...
 * @param pCase
 */
function showSelectWin(param){//1.0.11新版本
	try {	if(param.x1>param.x2){
		var a=param.x2;
		param.x2=param.x1;
		param.x1=a;
	}
...
//	showMes("param_lsit:"+JSON.stringify(param_list));
	if(param_list!=null&&param_list!=""&&param_list.length>0&&(addType==null||addType==0)){
		for(var i=0;i<xmlEntities.length;i++){
 ...
 * @param param
 */
function addCase(param){//1.0.11新版本
	try {
		
		if(param.x2-param.x1>20&&param.y2-param.y1>20){
...
//				clearPlayType(0);//清理数据
                ...
                var fileList = lc.onGetFIle(1);
                document.location = 'linkyview://firstFile=' + fileList;
			});
		...
		if(decoders!=null){
			for(var i=0;i<decoders.length;i++){
				if(decoders[i].id==pii&&decoders[i].gn!=null&&
				   decoders[i].gn!=""&&decoders[i].gn!=0&&pic==0){
					pic=1;
				}
			}
		}
...
// 	console.log("点击开始："+getThisTime().minutes+"-"+ getThisTime().seconds+"-"+getThisTime().millis,"red");
	if(addType==2){
		//如果是在修改漫游状态，取消点击事件
		addType=0;//同时清理修改状态
		return;
...
//	showMes("bcType:"+bcType);
	...
	if((redNum>1&&(e.which==3 || e.which==0))||(act==1&&color=="red")){//判断选择了多个并且右键点击的时候
		return;
	}
	if (objType == 0) {
		// var keyType = null;
...
//							showMes("x1:"+xmlEntities[i].x1+" y1:"+xmlEntities[i].y1+" x2:"+xmlEntities[i].x2+"  y2:"+xmlEntities[i].y2 ,"red")	;
							...
							if(oilist!=null&&inputs!=null){
								...
								for(var j=0;j<oilist.length;j++){
									if(oilist[j].oi==id){
										...
										theDevice = oilist[j];// 设置当前操作输入源对象//1.0.11新版本
										
										for(var a=0;a<inputs.length;a++){//1.0.11新版本
											if(inputs[a].id==oilist[j].ii){	
												var stateObj = new Object();
												stateObj.win = showJSTxt(indexJSList,24)+"："+winNum;
												stateObj.type = showJSTxt(indexJSList,25)+"："+showtype;
												stateObj.inputName = showJSTxt(indexJSList,26)+"："+inputs[a].name;
												stateObj.inputAddress = showJSTxt(indexJSList,27)+"："+inputs[a].location;
												
												if(oilist[j].gn!=0&&isNaN(oilist[j].gn)==false){
													var s_n=oilist[j].gn;
													if(inputs[a].type==1){
														if(s_n%2==0){
															s_n=(s_n/2)+" ("+showJSTxt(indexJSList,3)+")";
							    						}else{
							    							s_n=(parseInt(s_n/2)+1)+" ("+showJSTxt(indexJSList,2)+")";
							    						}
													}
													stateObj.gn = showJSTxt(indexJSList,28)+": "+s_n;
							
												}else{
												}
												console.log("oilist[j].tp:" + (oilist[j].tp != " " ) + "  " + (oilist[j].tp != null));
												var tp = (oilist[j].tp != " " && oilist[j].tp != null) ? oilist[j].tp : "0*0";
												stateObj.size = showJSTxt(indexJSList,29)+"："+tp;
												
												LK_CAN_OBJ.onExt(3,"01 01",stateObj);// 调用回调接口
											}
										}
									}
								}
			...
			// console.log("计算完毕："+getThisTime().minutes+"-"+ getThisTime().seconds+"-"+getThisTime().millis,"red");
			if(show_list!=null){
			...
			// console.log("画图结束："+getThisTime().minutes+"-"+ getThisTime().seconds+"-"+getThisTime().millis,"red");
//			get_voice(id);//查看音量
		}
		param_list=null;
	}
...
function showDivAdd(paramId){
	try {
		...
		if(txt_type==0){
			$("#s_r").attr("src","/Public/common/api/img/jr_5.png");
			$("#d_l").attr("src","/Public/common/api/img/cs_5.png");
		}else if(txt_type==1){
			$("#s_r").attr("src","/Public/common/api/img/ejr_5.png");
			$("#d_l").attr("src","/Public/common/api/img/ecs_5.png");
		}
...
//				showMes("新IFOlist:"+JSON.stringify(minIFO),"blue");
			    ...
			    var iStr=(IFOlist!=null)?getExt(1,"IFOlist",IFOlist):"";
				LK_CAN_OBJ.onExt(1,7,iStr);//调用回调接口
                LK_CAN_OBJ.onExt(1,30,rightId);//回调清空窗口p2p指令
...
*
*/
function getCanXY(id){//1.0.11新版本
	try {
		...
		var mx = 0;
		var my = 0;
		var mouseX=0;// 鼠标相对页面开始的X坐标
		var mouseY=0;// 鼠标相对页面开始的Y坐标
		try {//移动端获取手指坐标
			document.createEvent("TouchEvent");
			var touch = tEvent.touches[(tEvent.touches.length - 1)]; // 获取第一个触点
			console.log(JSON.stringify(tEvent));
			mx = Number(touch.pageX); // 页面触点Y坐标
			my = Number(touch.pageY); // 页面触点Y坐标
			mouseX=mx;// 鼠标相对页面开始的X坐标
			mouseY=my;// 鼠标相对页面开始的Y坐标
		} catch (e) {//pc端获取鼠标坐标
			console.log("getCanXY不支持滑动功能！"+e);
			mx = event.x;
			my = event.y;
			mouseX=(mx+hideX);// 鼠标相对页面开始的X坐标
			mouseY=(my+hideY)+21;// 鼠标相对页面开始的Y坐标
		}
		
		var canvaX=canva.offsetParent.offsetLeft+canva.offsetLeft;
		var canvaY=canva.offsetParent.offsetTop+canva.offsetTop;
		// alert("mouseX:"+mouseX+" mouseY:"+mouseY+" canvaX:"+canvaX+"
		// canvaY:"+canvaY);
		index.x=mouseX-canvaX;
		index.y=mouseY-canvaY;
		
		var paper_middle=document.getElementById("paper-middle");
		if(paper_middle!=null){
			
			// console.log("x2:"+paper_middle.offsetLeft+"
			// y2:"+paper_middle.offsetTop);
		}
		
		return index;
	} catch (e) {
		console.log("getCanXY:"+e);
		setError("getCanXY", e);
	}
...
//		showMes("对应关系开始："+getThisTime().minutes+"-"+ getThisTime().seconds+"-"+getThisTime().millis+"  总计算4："+(endNum3-openNum),"blue");
	...
	{"id":"110","E_val":"YTControl","F_val":"雲台控制","C_val":"云台控制"}
	];
...
//	       {"id":"109","E_val":"","F_val":"","C_val":""},//
	 ...
	 (function($) {
	 	var D = $(document).data("func", {}); //给document对象添加 func函数 
	 	$.smartMenu = $.noop;
	 	$.fn.smartMenu = function(data, options) {// $.fn = 给jquery对象添加 smartMenu方法
	 		
	 		var B = $("body"), defaults = {  // B =body div对象
	 			name : "",
	 			offsetX : 2,
	 			offsetY : 2,
	 			textLimit : 30,
	 		...
	 		var htmlCreateMenu = function(datum) {
	 			...
	 			if ($.isArray(dataMenu) && dataMenu.length) {
	 				htmlMenu = '<div style="max-height: 180px; overflow: auto;" id="smartMenu_' + nameMenu + '" class="'
...
//	 							text=updataText(text);
	 			...
	 			if (!jqueryMenu.size()) {
	 				console.log("funSmartMenu---------size:"+jqueryMenu.size());
	 				...
	 				console.log("funSmartMenu---------name:"+params.name +"  idKey:"+idKey);
	 		...
	 		$(this).each(function() {
	 			...
	 			console.log("---------------LONG_DOWN:"+LONG_DOWN);
	 			
	 			if(LONG_DOWN ){
	 				try {
	 					showMes("LONG_EVENT:"+LONG_EVENT.clientX);
	 					if ($.isFunction(params.beforeShow)) {
		 					params.beforeShow.call(this);
		 					//console.log("beforeShow:"+JSON.stringify(params)+"  this:"+);
		 				}
	 					var e = LONG_EVENT;
//	 					e.cancelBubble = true;
...
//		 				}
		 				...
		 				console.log("jqueryMenu:"+jqueryMenu);
		 				if (jqueryMenu) {
		 					jqueryMenu.css({
		 						display : "block",
	 			...
	 			this.oncontextmenu = function(e) {
	 				...
	 				console.log("params.beforeShow："+params.beforeShow+"  this:"+JSON.stringify(this));
	 				// 回调
	 				if ($.isFunction(params.beforeShow)) {
	 					params.beforeShow.call(this);
	 					//console.log("beforeShow:"+JSON.stringify(params)+"  this:"+);
	 				}
	 				e = e || window.event;//鼠标对象
	 				// 阻止冒泡
	 				e.cancelBubble = true;
	 				console.log("x:"+e.clientX+"  y:"+e.clientY);
	 				console.log("e.stopPropagation:"+JSON.stringify(e));
	 				if (e.stopPropagation) {
	 					e.stopPropagation();
	 				}
	 				// 隐藏当前上下文菜单，确保页面上一次只有一个上下文菜单
